...
**Answer:** The _store_ holds state, which is an object containing all of the changes made within the application _Actions_ are also objects; they hold information (type and payload data usually). The type and payload tell us what kind of change is occuring and what data it holds, if any. Actions are passed into functions called _reducers_ that take that information and update the state accordingly, which is held in the store and thus completes the cycle.

- [x] What is the difference between Application state and Component state? When would be a good time to use one over the other?

**Answer:** Application state has a global scope while component state has a local scope to that component. If you are making a form, for example, you will need a local state to keep track of the text in the input element. You don't need this information globally. All you need globally is the end result chosen by the user. Until the end result is decided, the only thing that needs to keep track of every single change is the text box itself.

- [x] Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?

**Answer:** _redux-thunk allows action creators to return functions instead of just objects._ It's a middleware, the middleman between the action creator and the reducer. The action creator returns an action function, instead of the default action object; that function receives the dispatch method from the store. This dispatch method is used to complete actions inside the body of the function when we are done waiting for some data (usually from an API it seems). A thunk is a function used to delay the calcuation of an operation, so redux-thunk allows us to use React and Redux together asynchronously.
...
- [xIncor] **RUN** `yarn start` to fire up your React application. There ought to be a pretty little message awaiting you welcoming you to the app. `Follow` the prompting.
